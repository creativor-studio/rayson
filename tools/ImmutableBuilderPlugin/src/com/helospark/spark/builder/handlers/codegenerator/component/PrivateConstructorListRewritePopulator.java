...
public class PrivateConstructorListRewritePopulator {
	...
	private MethodDeclaration createPrivateConstructor(AST ast, TypeDeclaration originalType, TypeDeclaration builderType,
			List<NamedVariableDeclarationField> v) {
		Block body = ast.newBlock();
		/*
		 * for (NamedVariableDeclarationField field : v) { Assignment assignment
		 * = ast.newAssignment(); FieldAccess fieldAccess =
		 * ast.newFieldAccess();
		 ...
		 * builderFieldAccess.setExpression(ast.newSimpleName(
		 * classNameToVariableNameConverter.convert(builderType.getName().
		 * toString())));
		...
		// SingleVariableDeclaration methodParameterDeclaration =
